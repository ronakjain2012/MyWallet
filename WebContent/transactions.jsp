...
<title>Income | My Personal Wallet V.1</title>
<style type="text/css">
#empty_div {
float: left;
width: 5in;
...
<body>
	...
	%>
	<div id="empty_div"> <br><br><br><br><br> Income :) </div>
	<div id="transaction">
	...
	<table border="0" cellspacing="15" cellpadding="5.1">
		<tr> <td> Amount </td>  <td> <input type="text" size="20"  Placeholder="Ammount (Number only)" name="T_ammount" maxlength="5" onkeypress="return isNumber(event)" required > &nbsp; <font color="gray" onclick="mycalc()"> Calculotor </font></td> </tr>  
		...
		<tr> <td> Paid to </td> <td> <select name="T_paidto" value="unknown" readonly="readonly"> <option value="unknown"> unknown </option> </select>
		<tr> <td> Paid by </td> <td> <select name="T_paidby">
		<% try {
		Class.forName("com.mysql.jdbc.Driver");
		con = DriverManager.getConnection("jdbc:mysql://localhost:3306/wallet","root","root");
		String val = null; 
		PreparedStatement ps = con.prepareStatement("select * from paidby");
		ResultSet rs = ps.executeQuery();
			while(rs.next()) {
				val = rs.getString(2);
		%>
		...
		<%} } catch(Exception e) { 
			e.printStackTrace();
			response.sendRedirect("sqlError.jsp"); }  %>
		</select> <a href="addLedger.jsp"> Add Debtor </a> </td>  </tr>
	...
	<input type="hidden" value="income" name="T_type">
	</form>
	</div>
</body>
</html>